...
CONFIGURATION="${1:-all}"
RUST_REPO_PATH="${SCRIPTING_ENGINE_RUST_PATH:-}"
...
resolve_rust_repo() {
  ...
  local candidate
  candidate="$(resolve_root)/repos/scripting-engine-rust"
  if [[ -d "$candidate" ]]; then
    echo "$candidate"
    return
  fi

  echo "ERROR: Could not locate scripting-engine-rust. Set SCRIPTING_ENGINE_RUST_PATH." >&2
  exit 1
}

resolve_package() {
  local candidate
  candidate="$(resolve_root)/repos/scripting-engine-unity"
  if [[ -d "$candidate" ]]; then
    echo "$candidate"
    return
  fi

  echo "ERROR: Could not locate scripting-engine-unity package." >&2
  exit 1
}
...
build_one() {
  ...
  case "$os" in
    Linux*)
      lib_name="libse_ffi.so"
      ;;
    Darwin*)
      lib_name="libse_ffi.dylib"
      ;;
    *)
  ...
  (cd "$rust_repo" && cargo build -p se-ffi ${profile:+--$profile})
